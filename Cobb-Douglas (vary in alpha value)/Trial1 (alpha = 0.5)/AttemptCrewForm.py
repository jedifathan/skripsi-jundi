...
def AttemptCrewFormation(a, b, A, B, W0, Wc, d, mE, Recruitable,\
	...
	if (agenttype=='aff'):
		...
		P_trans = ((E_new**(1.75))*(T_new**(2-1.75)) - (E_old**(1.75))*(T_old**(2-1.75)))/K
	elif (agenttype=='pow'):
		# P_trans = pow_motive(DeltaGK)
		P_trans = ((E_new**(.5))*(T_new**(2-.5)) - (E_old**(.5))*(T_old**(2-.5)))/K
	else:
		P_trans = DeltaGK
		...
		if ( len(InitialCrew)<ImplementSettings[Gamma[b]][1] ):
			for a1 in ProspectiveCrew:
				...
				if (agenttype1=='aff'):
					...
					P_trans = ((E_new1**(.5))*(T_new1**(2-.5)) - (E_old1**(.5))*(T_old1**(2-.5)))/K
				else:
					P_trans = DeltaGK1
